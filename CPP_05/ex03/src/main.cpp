...
{
	printTitle("testIntern START", 60);
	{
		std::string formTypes[] = {"shrubbery creation", "robotomy request", "presidential pardon", "Unknown"};
		...
		{
			printTitle(formTypes[i], 80);
			rrf = someRandomIntern.makeForm(formTypes[i], "Bender");
			if (rrf)
			{
				b1.signForm(*rrf);
				b1.executeForm(*rrf);
...
int	main(int ac, char* arg[])
{
	...
	if (ac == 2)
	{
		...
		switch (choice)
		{
			...
			case 9: tryCatch(&testIntern, os); break;
